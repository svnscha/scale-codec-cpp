...
public:
   ...
   * @return reference to stream
   */
  template <class T> ScaleEncoderStream &operator<<(const std::vector<T> &c) {
   ...
   * @return reference to stream
   */
  template <class T> ScaleEncoderStream &operator<<(const std::list<T> &c) {
    return encodeCollection(c.size(), c.begin(), c.end());
  }
   ...
   * @return reference to stream
   */
  template <class T> ScaleEncoderStream &operator<<(const gsl::span<T> &v) {
